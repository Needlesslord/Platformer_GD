...
j1Map1::j1Map1() : j1Module(), map_loaded(false) {
	name.create("map");
}
...
bool j1Map1::Awake(pugi::xml_node& config) {
...
void j1Map1::Draw() {
	...
	for (uint y = 0; y < data.layers.count(); y++) {
		if (data.layers[y]->mapname != "Navigation") {
				...
				for (uint i = 0; i < data.height; i++) {
					for (uint j = 0; j < data.width; j++) {
						...
						if (data.layers[y]->parallaxSpeed == 1) {
							if (wCoord.x >= camera.x - 32 && wCoord.x <= camera.x + camera.w && wCoord.y >= camera.y - 32 && wCoord.y <= camera.y + camera.h) App->render->Blit(data.tilesets[x]->texture, j * data.tile_width, i * data.tile_height, &data.tilesets[x]->GetTileRect(data.layers[y]->data[data.layers[y]->Get(j, i)]), SDL_FLIP_NONE, -data.layers[y]->parallaxSpeed);
						}
						else App->render->Blit(data.tilesets[x]->texture, j * data.tile_width, i * data.tile_height, &data.tilesets[x]->GetTileRect(data.layers[y]->data[data.layers[y]->Get(j, i)]), SDL_FLIP_NONE, -data.layers[y]->parallaxSpeed);
					}
				}
			}
		}
...
iPoint j1Map1::MapToWorld(int x, int y) const {
	iPoint ret;
...
// Load new map
bool j1Map1::Load_map(const char* file_name) {
	bool ret = true;
...
bool j1Map1::LoadTilesetDetails(pugi::xml_node& tileset_node, TileSet* set) {
	bool ret = true;
...
bool j1Map1::LoadTilesetImage(pugi::xml_node& tileset_node, TileSet* set) {
	bool ret = true;
...
bool j1Map1::LoadLayer(pugi::xml_node& node, MapLayer* layer) {
	layer->mapname = node.attribute("name").as_string();
	...
	for (layer_node = node.child("data").child("tile"); layer_node; layer_node = layer_node.next_sibling("tile")) layer->data[i++] = layer_node.attribute("gid").as_uint(0);

	return true;
}

bool j1Map1::LoadObjectLayers(pugi::xml_node& node, ObjectsGroup* group) {
	bool ret = true;
...
int MapProperties::Get(const char* value, int default_value) const {
	p2List_item<MapProperty*>* item = list.start;

	while (item) {
		if (item->data->mapname == value) return item->data->value;
		item = item->next;
	}
...
bool j1Map1::LoadProperties(pugi::xml_node& node, MapProperties& properties) {
	bool ret = false;
	...
	if (data != NULL) {
		...
		for (prop = data.child("property"); prop; prop = prop.next_sibling("property")) {
			MapProperties::MapProperty* p = new MapProperties::MapProperty();
			p->mapname = prop.attribute("name").as_string();
...
bool j1Map1::SwitchMaps(p2SString* new_map) {
	CleanUp();
...
TileSet* j1Map1::GetTilesetFromTileId(int id) const {
	p2List_item<TileSet*>* item = data.tilesets.start;
